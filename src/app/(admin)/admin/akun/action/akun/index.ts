"use server"
import { createAccountWithClerk, deleteAccount, suspendAccount, activateAccount, updateAccount } from "@/services/accountServices";
...
export async function createAction(formData: FormData) {
    ...
    try {
        ...
        return { ok: false, message: e?.message || "Failed to create account." };
    }
}
...
export async function updateAction(id: number, formData: FormData) {
    ...
    try {
        await updateAccount(id, { email, password, fullName, company, position, phoneNumber, address, role, status });
        return { ok: true, message: "Account updated successfully." };
    } catch (e: any) {
        return { ok: false, message: e?.message || "Failed to update account." };
    }
}
