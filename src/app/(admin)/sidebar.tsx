...
import {
  HandCoins, Route as RouteIcon, Factory, FileText, BarChart3,
  ...
  return (
    <Link
      ...
      className={[
        ...
        active ? 'bg-emerald-600/20 text-white shadow-inner'
  ...
  return (
    <div className="flex h-full w-64 flex-col bg-gradient-to-b from-emerald-600 to-emerald-700 text-white">
      ...
      <ScrollArea className="h-[calc(100vh-180px)] px-2 py-3">
        <p className="px-3 pb-2 text-xs font-semibold uppercase tracking-wide text-emerald-100/70">Transaction</p>
        <div className="space-y-1 px-2">
          <NavItem href="/admin/transaction" icon={HandCoins} label="Transaction" />
          <NavItem href="/admin/trace" icon={RouteIcon} label="Trace" />
          <NavItem href="/admin/declaration" icon={Factory} label="Declaration" />
        </div>

        <p className="px-3 pt-4 pb-2 text-xs font-semibold uppercase tracking-wide text-emerald-100/70">Report</p>
        <div className="space-y-1 px-2">
          <NavItem href="/admin/report/summary-cpo-pk" icon={FileText} label="Summary CPO & PK" />
          {/* contoh item lain */}
          <NavItem href="/admin/report/dashboard" icon={BarChart3} label="Dashboard" />
        </div>
      </ScrollArea>
    </div>
  )
